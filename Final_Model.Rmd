...
```{r}
ghg<- for (i in length(out_clean$time)){
  out_clean$miles_driven = avg_mile_day + weather_coeff * (out_clean$temp - mean(out_clean$temp)) + shelter_coeff * (out_clean$V6)
...
init <- c(S =1-.001, E =.001, I = 0, R = 0, rho) # assuming 0.1% of the population has been infected
parms <- c(alpha = 0.2, beta = 2, gamma = 1)

# Run model for all parameter sets
res = mapply(FUN = SEIR,
             gamma = covid_sens$X$gamma,
             beta = covid_sens$X$betta,
             MoreArgs = list(time = 180,
                             S = 1-.001,
                             E = 0.001,
                             I = 0,
                             R = 0,
...
SEIR_sobel = sensitivity::tell(covid_sens, res, res.names="out")
