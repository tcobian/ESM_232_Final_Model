...
final_model<- function(time, i, avg_mile_day = 974219178, mile_emission = 0.411, vars){
  # make the parms list in the function
  b<- vars$b
  s<- vars$s
  k<- vars$k
  shelter_strength<- vars$shelter_strength
  mean_temp<- vars$mean_temp
...
# return the data as a list of covid  
  return(list(covid))
}
...
result = ode(y = i, times = time, func = final_model, parms = c(vars=parms))
```
